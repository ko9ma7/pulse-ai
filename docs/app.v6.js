(() => {
  const BUILD_VERSION = 'v6.0.0-backfill-20260909';
  ...
  const metricValue = (r,p=state.period) => {
    const direct = r[metricKey(p)];
    if (direct !== undefined && direct !== null && Number.isFinite(Number(direct))) return safeNum(direct);
    const days = daysByPeriod(p);
    return (r.history || []).slice(-days).reduce((sum, point) => sum + safeNum(point.gained), 0);
  };
  const periodGrowthPct = (r,p=state.period) => {
    const gain = metricValue(r,p);
    const base = Math.max(1, safeNum(r.stars) - gain);
  ...
  function shell(content, active='home'){
    ...
    return `<aside class="sidebar ${state.mobile?'open':''}" id="sidebar">
      ...
      <div class="sidebar-foot"><div class="sidebar-note">자동 수집: <strong>6시간 간격</strong><br>장기 Archive: <strong>최대 730일</strong><br>Reconstructed Backfill: <strong>1년</strong><br>Repo Star History: <strong>최대 1년</strong><br><small>UI ${BUILD_VERSION}</small></div></div>
    </aside>
  ...
  function backfillStatus(){
    ...
    const ready = Boolean(b?.generatedAt && b?.snapshots?.length && b?.summary?.repositoriesBackfilled);
    return {
      ready,
      repos: safeNum(b?.summary?.repositoriesBackfilled),
      weeks: safeNum(b?.summary?.weeklySnapshots),
  ...
  function dashboard(){
    ...
    const rows=repos.map((r,i)=>{
      ...
      return `<div class="repo-row">
        ...
        <div class="metric gain"><span>${periodLabel(state.period)} Growth</span><strong>+${fmt.format(metricValue(r))}</strong><small>${periodGrowthPct(r).toFixed(1)}%</small></div>
    ...
    const bf=backfillStatus();
    return shell(`<section class="page">
      <div class="hero"><div><h1>Today's Fastest Rising <span>AI Repositories</span></h1><p>최신 Radar는 6시간마다 갱신되고, 기간 버튼을 누르면 해당 기간의 <strong>Star Growth 순위</strong>로 자동 전환됩니다. 과거 1년 재구성 데이터는 Backfill 1Y에서 별도로 확인합니다.</p></div><div class="freshness"><i class="status-dot"></i><div><strong>Latest radar snapshot</strong><span>${freshnessText(d.generatedAt)} · 자동 6시간</span></div></div></div>
      <div class="backfill-strip ${bf.ready?'ready':'pending'}"><strong>${bf.ready?'1Y BACKFILL READY':'1Y BACKFILL NOT READY'}</strong><span>${bf.ready?`${fmt.format(bf.repos)} repos · ${fmt.format(bf.weeks)} weekly snapshots · ${freshnessText(bf.generatedAt)}`:'아직 historical-backfill.json에 실제 1년 데이터가 없습니다. Backfill Action을 완료해야 합니다.'}</span><a href="#/historical">${bf.ready?'과거 1년 보기 →':'Backfill 상태 확인 →'}</a></div>
  ...
  function historicalPage(){
    ...
    if(!has){
      return shell(`<section class="page"><div class="hero"><div><h1>Historical <span>Backfill</span></h1><p>GitHub 공식 Star History를 이용해 지난 1년의 성장 모멘텀을 역산하는 영역입니다. 현재 배포된 historical-backfill.json에는 아직 실제 1년 백필 결과가 없습니다.</p></div></div><div class="archive-start"><strong>1년 백필이 아직 없습니다.</strong><span>Backfill Action이 성공 완료되어 generatedAt, repositoriesBackfilled, weeklySnapshots가 기록되어야 이 화면이 활성화됩니다. 아래 Action을 실행한 뒤 완료까지 기다리세요.</span></div><div class="quick-tabs"><a href="https://github.com/ko9ma7/pulse-ai/actions/workflows/backfill-year.yml" target="_blank" rel="noreferrer">↻ 1년 Backfill Action 실행</a><a href="./data/historical-backfill.json" download>↓ Backfill JSON</a></div></section>`,'historical');
    }
  ...
  const cacheKey = `v6-${Date.now()}`;
  Promise.all([
    fetch(`./data/repositories.json?${cacheKey}`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`repositories.json HTTP ${r.status}`);return r.json();}),
    fetch(`./data/radar-history.json?${cacheKey}`,{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null),
    fetch(`./data/historical-backfill.json?${cacheKey}`,{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null),
  ]).then(([data,archive,backfill])=>{state.data=data;state.archive=archive;state.backfill=backfill;state.backfillLoaded=true;render();}).catch(err=>{app.innerHTML=`<div class="loading-screen"><strong>Radar 데이터를 불러오지 못했습니다.</strong><span>${esc(err.message)}</span><button class="primary" onclick="location.reload()">다시 시도</button></div>`;});
})();
